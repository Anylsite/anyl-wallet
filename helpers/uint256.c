...
void readu64BE(const uint8_t *buffer, uint128_t *target) {
    UPPER_P(target) = 0;
    LOWER_P(target) = * (uint64_t*)buffer;
}
...
{
    writeu128BE(&number->elements[0], buffer);
    writeu128BE(&number->elements[1], buffer + sizeof(uint128_t));
}
...
void divmod128(
        const uint128_t *l, const uint128_t *r,
        uint128_t *retDiv, uint128_t *retMod
        )
{
...
void divmod256(
        const uint256_t *l, const uint256_t *r,
        uint256_t *retDiv, uint256_t *retMod
)
{
...
int fromstring256(const char *hexencoded, uint256_t *out)
{
    ...
    if((strlen(hexencoded) < 2) || (strlen(hexencoded) > (64))) {
        return -1;
    }
    ...
    hextobin(hexencoded, buf_p, sizeof(buf) - (buf_p - buf));
